...
void correctAlpha(GSTEXTURE *tex) {
  ...
  for (u32 i = 0; i < num_pixels; i++) {
    ...
    // Invertir el canal Alpha: Alpha_Nuevo = 0x80 - Alpha_Original (o 255 -
    // alpha, usando 0x80 por el contexto)
...
int loadCoverArt(struct DeviceMapEntry *device, char *titleID) {
  ...
  icoTexture->Mem =
      (void *)1; // Temporalmente marcado como "no cargado/no disponible"
  ...
  if (gsKit_texture_png(gsGlobal, icoTexture, lineBuffer) == 0) {
    ...
    // Posición X final: solapa 64px (la mitad del ícono) en la izquierda de
    // coverArtX1
    ...
    // Falló la carga de ICO.png. icoTexture->Mem ya está marcado como (void*)1
    // (no cargado). Se asegura que icoTexture->Mem no sea NULL para
    // distinguirlo de "Cargado en VRAM".
  }
...
int uiLoop(TargetList *titles) {
  ...
  void drawTitleList(TargetList * titles, int selectedTitleIdx,
    ...
    // Las variables ya están declaradas como static globales al inicio del
    // archivo.
    // ----------------------------------------------------
    ...
    if (selectedTitleCover != NULL) {
      ...
      gsKit_prim_sprite(gsGlobal, coverArtX1, coverArtY1, coverArtX2,
                        coverArtY2, 1, currentTheme.background);
  ...
  if (icoTexture != NULL) {
    ...
    if (icoTexture->Mem == NULL) {
      ...
      gsKit_prim_sprite_texture(gsGlobal, icoTexture, (float)icoArtDrawX1,
                                (float)icoArtDrawY1, 0.0f, 0.0f,
                                (float)icoArtDrawX2, (float)icoArtDrawY2,
                                3, // Z-depth (tercer plano)
                                icoTexture->Width, icoTexture->Height,
                                FontMainColor); // Color (12mo argumento)
    } else {
