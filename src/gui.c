...
#define ICO_ART_RES 128 // <--- NUEVO: Resolución 128x128 para icoArt
...
// (PSMCT32). Asume el formato AARRGGBB o similar.
void correctAlpha(GSTEXTURE *tex) {
  ...
  for (u32 i = 0; i < num_pixels; i++) {
    ...
    // Extraer el Alpha (bits 24-31)
    u8 alpha = (u8)((pixel_value >> 24) & 0xFF);

    // Invertir el canal Alpha: Alpha_Nuevo = 0x80 - Alpha_Original (Ajuste
    // común para blending en PS2)
...
static GSTEXTURE *icoTexture; // <--- NUEVO: Textura para icoArt
...
// <--- NUEVO: Coordenadas e información de animación para icoArt
static int icoArtFinalX;    // Posición final del borde izquierdo de icoArt
static int icoArtY;         // Posición Y superior fija
static float icoArtScrollX; // Posición X actual para la animación
static int icoArtAnimationState = 0; // 0: Espera, 1: Scroll, 2: Detenido
static int icoArtFrameCounter = 0;   // Contador de frames para la espera
...
int uiInit() {
  ...
  // ************************************************
  // Inicialización y cálculo de coordenadas de coverArt
  // ************************************************
  coverTexture->PSM = GS_PSM_CT32;
  coverTexture->Delayed = 1;

  // ************************************************
  // Inicialización y cálculo de coordenadas de icoArt (NUEVO)
  // ************************************************
  icoTexture = calloc(sizeof(GSTEXTURE), 1); // <--- NUEVA ASIGNACIÓN
  icoTexture->PSM = GS_PSM_CT32;
  icoTexture->Delayed = 1;

  // Posición Y superior: Alineado al borde inferior de coverArt
  icoArtY = coverArtY2 - ICO_ART_RES;

  // Posición final X: Borde izquierdo de coverArt (coverArtX1) + 64px de
  // solapamiento - ancho de icoArt (128px) Coordenada del borde derecho de
  // icoArt = coverArtX1 + 64 Coordenada del borde izquierdo de icoArt (FinalX)
  // = (coverArtX1 + 64) - ICO_ART_RES
  icoArtFinalX = coverArtX1 + 64 - ICO_ART_RES;

  // Posición inicial X (para la animación): Empezar 10px fuera de la posición
  // final (a la izquierda)
  icoArtScrollX = (float)icoArtFinalX - ICO_ART_RES - 10.0f;
...
int loadCoverArt(struct DeviceMapEntry *device, char *titleID) {
  ...
  // Liberar memoria tras la subida a VRAM
  if (coverTexture->Mem) {
  ...
  // Resetear el estado de la animación cuando se carga un nuevo título
  icoArtScrollX = (float)icoArtFinalX - ICO_ART_RES -
                  10.0f; // Posición inicial fuera de pantalla
  icoArtAnimationState = 0;
  icoArtFrameCounter = 0;
...
void closeUI() {
  ...
  closeFont();
  free(coverTexture);
  free(icoTexture); // <--- NUEVO: Liberar la textura ico
  gsKit_deinit_global(gsGlobal);
}

// Main UI loop. Displays the target list.
int uiLoop(TargetList *titles) {
  // ... (parte inicial sin cambios)
  ...
  int isCoverUninitialized =
      1; // Ya no se usa directamente para el ico, pero se mantiene para cover
  ...
  // Load cover art (y también icoArt)
  ...
  while (1) {
    ...
    // Draw title list
    // Ahora pasamos coverTexture; drawTitleList se encargará de
    // icoTexture/Animación.
    drawTitleList(titles, selectedTitleIdx, maxTitlesPerPage, coverTexture);

    // ... (el resto del bucle uiLoop sigue sin cambios)
...
void drawTitleList(TargetList *titles, int selectedTitleIdx,
  ...
  // ----------------------------------------------------
  // ACCESO A VARIABLES GLOBALES DE ICOART (Asumidas de Parte 1/2)
  // - GSTEXTURE *icoTexture
  // - float icoArtScrollX;
  // - int icoArtAnimationState;
  // - int icoArtFrameCounter;
  // - int icoArtY;
  // - int icoArtFinalX;
    ...
    if (icoTexture->Mem == NULL) {
      ...
      case 0: // INICIAL: Espera 30 frames
        ...
        if (icoArtFrameCounter >= START_DELAY_FRAMES) {
          ...
          // icoArtScrollX ya fue inicializado en loadCoverArt() a la posición
          // de inicio
        }
        break;
      ...
      case 1: // SCROLL: Movimiento hacia la derecha
        ...
        // Verificar posición de parada: Solapar 64px de coverArtX1
        // Límite de parada = icoArtFinalX
  ...
  // Se dibuja después de coverArt, si es necesario solapar por encima.
  // Si debe solapar POR DEBAJO de coverArt, este bloque debe ir antes de
  // coverArt. Asumiendo que debe solapar por encima, se deja aquí.
  // ----------------------------------------------------
  ...
  if (icoTexture != NULL) {
    ...
    if (icoTexture->Mem ==
        NULL) { // Mem == NULL indica que la textura está en VRAM y lista
      gsKit_prim_sprite_texture(
          ...
          3, // Prioridad 3 (sobre cover art)
          icoTexture->Width, icoTexture->Height, 3, FontMainColor);
    } else {
      // Placeholder si falla la carga o aún está en RAM (no debería pasar con
      // el patrón usado)
  ...
  void drawTitleOptionsFooter(int baseX) {
    ...
    drawIconWindow(baseX + getIconWidth(ICON_CIRCLE), baseY, 0,
                   gsGlobal->Height, 0, currentTheme.iconCross, ALIGN_CENTER,
                   ICON_CROSS);
    ...
    drawTextWindow(
        0, gsGlobal->Height - 1 - footerHeight - getFontLineHeight() / 2,
    ...
    while (1) {
      ...
      for (i = 0; i < uiArgumentsTotal; i++) {
        startY = getFontLineHeight() / 2 +
                 uiArguments[i].draw(&uiArguments[i],
                                     (i == activeArgumentIdx) ? 1 : 0, baseX,
                                     startY, 0, gsGlobal->Width - baseX, 0);
      }
      ...
      if (input & (PAD_L1 | PAD_R1)) {
        ...
        case ACTION_CHANGED:
          uiArguments[activeArgumentIdx].marshal(
              &uiArguments[activeArgumentIdx], titleArguments);
          break;
  ...
  int uiArgumentListLoop(Target * target, ArgumentList * titleArguments) {
    ...
    while (1) {
      ...
      // Set number of elements per page according to line height and
      ...
      if (input & (PAD_L1 | PAD_R1)) {
        ...
        // Launch title without saving arguments
        uiLaunchTitle(target, titleArguments);
        return -1; // If this was somehow reached, something went terribly
                   // wrong
      } else if (input & PAD_START) {
        updateTitleLaunchArguments(target, titleArguments);
        return 1;
  ...
  struct {
    ...
    int32_t newStringSema; // Used to signal UI splash thread that a new
                           // string is ready
...
#define THREAD_STACK_SIZE 0x1000
  ...
  void uiSplashThread() {
    ...
    drawTextWindow(
        0, (gsGlobal->Height / 4 + getLogoHeight() + 10), gsGlobal->Width, 0, 0,
        GS_SETREG_RGBA(0x40, 0x40, 0x40, 0x80), ALIGN_HCENTER, SKIN_EDITION);
    gsKit_mode_switch(gsGlobal, GS_ONESHOT);
