...
// Asume el formato AARRGGBB o similar.
void correctAlpha(GSTEXTURE *tex) {
    ...
    for (u32 i = 0; i < num_pixels; i++) {
        ...
        // Invertir el canal Alpha: Alpha_Nuevo = 0x80 - Alpha_Original (Ajuste común para blending en PS2)
...
int uiInit() {
    ...
    // Posición final X: Borde izquierdo de coverArt (coverArtX1) + 64px de solapamiento - ancho de icoArt (128px)
    // Coordenada del borde derecho de icoArt = coverArtX1 + 64
    // Coordenada del borde izquierdo de icoArt (FinalX) = (coverArtX1 + 64) - ICO_ART_RES
    icoArtFinalX = coverArtX1 + 64 - ICO_ART_RES; 
    
    // Posición inicial X (para la animación): Empezar 10px fuera de la posición final (a la izquierda)
...
int loadCoverArt(struct DeviceMapEntry *device, char *titleID) {
    ...
    icoArtScrollX = (float)icoArtFinalX - ICO_ART_RES - 10.0f; // Posición inicial fuera de pantalla
    icoArtAnimationState = 0;
    icoArtFrameCounter = 0;
...
int uiLoop(TargetList *titles) {
    ...
    int isCoverUninitialized = 1; // Ya no se usa directamente para el ico, pero se mantiene para cover
    ...
    while (1) {
      ...
      // Draw title list
      // Ahora pasamos coverTexture; drawTitleList se encargará de icoTexture/Animación.
...
void drawTitleList(TargetList *titles, int selectedTitleIdx,
            ...
            switch (icoArtAnimationState) {
                case 0: // INICIAL: Espera 30 frames
                    ...
                    if (icoArtFrameCounter >= START_DELAY_FRAMES) {
                        ...
                        // icoArtScrollX ya fue inicializado en loadCoverArt() a la posición de inicio
                    }
                    break;
    ...
    // Si debe solapar POR DEBAJO de coverArt, este bloque debe ir antes de coverArt.
    // Asumiendo que debe solapar por encima, se deja aquí.
    // ----------------------------------------------------
    ...
    if (icoTexture != NULL) {
        ...
        if (icoTexture->Mem == NULL) { // Mem == NULL indica que la textura está en VRAM y lista
            gsKit_prim_sprite_texture(gsGlobal, icoTexture, (float)icoArtDrawX1,
                                      (float)icoArtDrawY1, 0.0f, 0.0f, (float)icoArtDrawX2,
                                      (float)icoArtDrawY2, 3, // Prioridad 3 (sobre cover art)
                                      icoTexture->Width,
                                      icoTexture->Height, 3, FontMainColor);
        } else {
            // Placeholder si falla la carga o aún está en RAM (no debería pasar con el patrón usado)
